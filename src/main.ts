...
async function run(): Promise<void> {
  try {
    ...
    if (args.debug) {
      info(
        `${ansiColorsCode.cyan}Contenu de resultCompareErrors : ${JSON.stringify(
          resultCompareErrors
        )}`
      );
    }
    ...
    if (
      [OUTPUT_BEHAVIOUR.ANNOTATE, OUTPUT_BEHAVIOUR.COMMENT_AND_ANNOTATE].includes(
    ...
    if (
      [OUTPUT_BEHAVIOUR.COMMENT, OUTPUT_BEHAVIOUR.COMMENT_AND_ANNOTATE].includes(
        args.outputBehaviour
      )
    ) {
